...
struct GgmlQ4Weights {
    size_t K, N;
    std::vector<uint8_t> q4_data;
    size_t row_stride;

    // Pre-allocated for run_once (lazy-initialized on first call)
    size_t once_M = 0;
...
void* prepare(const float* fp32, size_t N, size_t K) {
    auto* w = new GgmlQ4Weights();
    w->K = K;
    w->N = N;
    w->row_stride = ggml_row_size(GGML_TYPE_Q4_0, K);
    w->q4_data.resize(w->row_stride * N);
    ...
    for (size_t n = 0; n < N; n++)
        from_float(fp32 + n * K, w->q4_data.data() + n * w->row_stride, static_cast<int64_t>(K));

    return w;
}

static void gemv_q4_slice(ggml_vec_dot_t vec_dot, int64_t nrows,
...
bench::BenchResult run(size_t M, void* weights, void*,
    ...
    auto* w = static_cast<GgmlQ4Weights*>(weights);
    const size_t K = w->K, N = w->N;

    const auto* cpu_traits_q4 = ggml_get_type_traits_cpu(GGML_TYPE_Q4_0);
    auto vec_dot = cpu_traits_q4->vec_dot;
    int64_t nrows = cpu_traits_q4->nrows;
    ...
    const uint8_t* wdata = w->q4_data.data();
    ...
    auto run_once = [&]() {
        CactusThreading::parallel_for(N, par_cfg,
            [&](size_t n_begin, size_t n_end) {
                gemv_q4_slice(vec_dot, nrows, wdata, w_stride,
    ...
    if (opt.capture_reference) {
        auto to_float_q4 = ggml_get_type_traits(GGML_TYPE_Q4_0)->to_float;
        auto to_float_q8 = ggml_get_type_traits(GGML_TYPE_Q8_0)->to_float;

        std::vector<float> deq_w(N * K);
        for (size_t n = 0; n < N; n++)
            to_float_q4(wdata + n * w_stride, deq_w.data() + n * K, static_cast<int64_t>(K));
...
void once(size_t M, void* weights, void*,
    ...
    if (w->once_M != M) {
        w->once_M = M;
        const auto* cpu_traits_q4 = ggml_get_type_traits_cpu(GGML_TYPE_Q4_0);
        w->vec_dot = cpu_traits_q4->vec_dot;
        w->nrows = cpu_traits_q4->nrows;
    ...
    CactusThreading::parallel_for(w->N, par_cfg,
        [&](size_t n_begin, size_t n_end) {
            gemv_q4_slice(w->vec_dot, w->nrows,
                          w->q4_data.data(), w->row_stride,
...
void cleanup(void* weights, void*) {
    delete static_cast<GgmlQ4Weights*>(weights);
}

static int reg = [] {
    bench::register_backend({
        "ggml_q4_0", "ggml", bench::QuantCategory::INT4,
